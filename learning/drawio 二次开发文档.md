Markdown Content:
> 本文作者：[溪岼](https://link.segmentfault.com/?enc=p03hmmno%2FPUARKgaHNLC7w%3D%3D.U07YR9FZfZ6zKYxmMqwE8wv9tGe7LjKrmsTvBwCaimU%3D)
...
![Image 64: overmind-x graph drawio](https://segmentfault.com/img/remote/1460000040793819)

![Image 65: overmind-x graph list](https://segmentfault.com/img/remote/1460000040793820)

DrawIO 简介
---------

既然是基于 DrawIO 进行二开，首先就来了解一下 DrawIO 到底是什么，跟着笔者视角先去 [DrawIO 官网](https://link.segmentfault.com/?enc=IDm9dpEqULJa%2BMSdZN55Wg%3D%3D.l4vi6l67NuOmaQU8vQoPBI2PPr1uBVcyfc%2B9gC9BbNY%3D)去看一圈，官网首页截图如下所示：

![Image 66: DrawIO 官网首页](https://segmentfault.com/img/remote/1460000040793821)
...
这里笔者新建了一个名为 [drawio-folder](https://link.segmentfault.com/?enc=llEDUTXlTfdRvbit7Ncjpg%3D%3D.AmsiCsOI6%2FGsWCBjf43XQAe2HicMsC47tWyf6UAUQWFHKPwZR7rA6ixIOe2xTjmU) 的仓库，里面只有一个 `README.md` 文件，如下图所示：

![Image 67: DrawIO Github Folder](https://segmentfault.com/img/remote/1460000040793822)

*   2 - 绘图时选择 GitHub 作为存储空间

然后从官网点击 Start 按钮进入绘图页面，第一次进入页面不出意外会出现一个如下图所示选择存储空间的选项，然后选择 Github（其他远程存储原理一样）。

![Image 68: DrawIO Github Service](https://segmentfault.com/img/remote/1460000040793823)

*   3 - 选择对应的仓库然后绘制图片

点击选择存储到 Github 仓库，然后选择新建一个图表，命名之后点击新建 Create 按钮完成图表的创建。

![Image 69: DrawIO Create Graph](https://segmentfault.com/img/remote/1460000040793824)

*   4 - 授权选取存储仓库

图标创建成功后，会出现一个授权登录的弹窗，授权登录过后 DrawIO 就能获取到账户下的所有仓库信息，选择刚才创建的 **drawio-folder** 仓库，然后进行图表的绘制。

![Image 70: DrawIO Select Folder](https://segmentfault.com/img/remote/1460000040793825)

*   5 - 绘图完毕保存文档

绘制过程中，每一次的保存操作，都会创建一条 `commit message`，就相当于正常的仓库修改并提交，如下图所示：

![Image 71: DrawIO Commit Message](https://segmentfault.com/img/remote/1460000040793826)

当一次图表绘制完成并保存后，可以去 Github 仓库看看是否存储成功。

![Image 72: DrawIO Git Result](https://segmentfault.com/img/remote/1460000040793827)
...
![Image 73: DrawIO Java Ant](https://segmentfault.com/img/remote/1460000040793828)

安装完成之后可以在命令行验证是否安装成功，如果命令行里出现了如上图所示对应的 Java 和 Ant 版本信息，那么恭喜你 DrawIO 二开的基础环境已经准备完成，接下来就是正式进入 DrawIO 二开了。

### DrawIO 源码

二开的开始，首先从 [DrawIO 源码仓库](https://link.segmentfault.com/?enc=s4lzXo2QhHqYDwfrrvizfQ%3D%3D.yX4z4xhE5F9UQcW8sVmvWSnHEFyTBPuxu3CJSQlDLL0cnnB51EziSpdTI%2BGN%2BIlk) Clone 官方代码，在本地跑一下看看效果。老规矩 Clone 下来先阅读 `package.json` 文件，发现其中只有一个 `start` 命令。
...
![Image 74: DrawIO Electron](https://segmentfault.com/img/remote/1460000040793829)

它的使用方式和上面文章里介绍的一样，但是问题来了！我们最终目标是要进行 DrawIO 二开，难不成是基于 Electron 二开吗？虽然说也不是不行，但是那上手成本可就高了，估计会劝退一大波同学，不要慌，去官方仓库看看有没有其他方法。

![Image 75: DrawIO Github Pages](https://segmentfault.com/img/remote/1460000040793830)
...
![Image 76: DrawIO Static Site](https://segmentfault.com/img/remote/1460000040793831)
...
![Image 77: DrawIO Dev Preview Code](https://segmentfault.com/img/remote/1460000040793832)

很容易理解，就是 Alert 一个弹窗提示测试一下代码是不是能正常执行，不过刷新页面之后会发现并没有看到这个弹窗，这是为什么呢？打开控制台找找原因：

![Image 78: DrawIO MinJs](https://segmentfault.com/img/remote/1460000040793833)
...
![Image 79: DrawIO Dev Param](https://segmentfault.com/img/remote/1460000040793834)
...
![Image 80: DrawIO Dev Error Reason](https://segmentfault.com/img/remote/1460000040793835)
...
![Image 81: DrawIO Dev Success](https://segmentfault.com/img/remote/1460000040793836)
...
![Image 82: DrawIO Build Success](https://segmentfault.com/img/remote/1460000040793837)

然后再来访问一下首页，这次不携带 `dev=1` 开发参数看看效果，如下图所示，非常完美的运行着新开发的生产版本，至此为止 DrawIO 的二开流程阶段，从开发预览到构件生产版本都已经介绍完成。

![Image 83: DrawIO Prod Success](https://segmentfault.com/img/remote/1460000040793838)
...
![Image 84: DrawIO Console File](https://segmentfault.com/img/remote/1460000040793839)
...
![Image 85: DrawIO FileSaver](https://segmentfault.com/img/remote/1460000040793840)
...
![Image 86: DrawIO LoadFile](https://segmentfault.com/img/remote/1460000040793841)
...
> 
> *   [个人版 DrawIO](https://link.segmentfault.com/?enc=h2306L72HxHNHo%2FMEfe9NQ%3D%3D.iRjyzC92AhEr5UEMktUiL41dQt3sxwSIP5O9hHRTRpfWRnfnzZEmXlUHsG2jq93F)
> *   [DrawIO 文件下载](https://link.segmentfault.com/?enc=xc4IrUOrABFqwigheuSUbQ%3D%3D.YeLkN5UHSwWld1ysxmQmYcZsBYHAEGRB8O4sa6%2FnESEUY8FqixdlgrBkPYk2vpsrxv6CnI30nb6yjPdioe6Lpxkmsy8HyIgI7HlxtL88ilw%3D)
> *   [DrawIO loadFile 加载文件](https://link.segmentfault.com/?enc=zDAQyUcnOqBdZrFoJKZY6A%3D%3D.Ki8bGn6XbbXqUOS465lNnEsB1dk3cXKkiTQeDgXtbORaEpiaRr63QWkK3%2F6pNmv3hfmzXpHWUPxEHfPQr%2B1hFA%3D%3D)
...
![Image 87: DrawIO Static Site Structure](https://segmentfault.com/img/remote/1460000040793842)
...
![Image 88: DrawIO Two System Structure](https://segmentfault.com/img/remote/1460000040793843)
...
![Image 89: DrawIO Special Structure](https://segmentfault.com/img/remote/1460000040793844)
...
那么主系统是通过何种方式完成绘图系统与业务系统的桥接呢？答案就是通过 Next.js 的 [Rewrites](https://link.segmentfault.com/?enc=ouYhAPtkY5PE9cZYZNwDyw%3D%3D.GXDmxBa4MJG0kGEb%2BUPylU3siIv3sic%2BizigcPPJgyrrjQjXp78y6keZCxdk5xIzcGzTcNicwsnuJ6f8kWXBqg%3D%3D) 特性，感兴趣的可以去查阅官方文档，正是因为这个特性才使得单系统进行 DrawIO 二开变成了可能并且开发起来也非常便利，**桥接双系统让整个工程对外形成一个独立完整系统**算是这个架构的核心之处。
...
> 讲这么多其实都帮各位看官准备好了，基于**架构三**搭建了一个脚手架 [simple-drawio-starter](https://link.segmentfault.com/?enc=uvKIVh5gYMTBagVHynwJKA%3D%3D.uxrG3k7YSt2iRxAaRlqYkMq%2BNQS285nMluymiDRPAXhDVZ5QE44TQ%2BZdT9tcefOix%2Bf2n%2BbekKl1p4irXSsY3Q%3D%3D)，大家直接拿来进行业务逻辑的开发即可。
